...
fn jaro_sim(x: &str, y: &str) -> f64 {
    ...
    let xl = xc.len(); let yl = yc.len();
    if xl == 0 && yl == 0 { return 1.0; }
...
fn jaro_winkler_sim(x: &str, y: &str, p: f64, max_l: usize) -> f64 {
    ...
    for (a, b) in x.chars().zip(y.chars()) {
        if a == b && l < max_l { l += 1; } else { break; }
    }
    js + l as f64 * p * (1.0 - js)
}

#[pyfunction(signature = (x, y))]
fn jaro_similarity(x: &str, y: &str) -> f64 { jaro_sim(x, y) }
